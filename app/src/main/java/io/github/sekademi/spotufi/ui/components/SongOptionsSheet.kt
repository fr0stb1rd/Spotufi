...
fun SongOptionsSheet(
    ...
    var downloaded by remember { mutableStateOf(io.github.sekademi.spotufi.data.preferences.isDownloaded(context, song.id.toString())) }
    var downloadingNow by remember { mutableStateOf(io.github.sekademi.spotufi.di.SongPlayer.isDownloading(song.url)) }
    var downloadPct by remember { mutableStateOf(io.github.sekademi.spotufi.di.SongPlayer.downloadProgress(song.url)) }
    ...
    androidx.compose.runtime.LaunchedEffect(downloadingNow) {
        while (downloadingNow) {
            downloadPct = io.github.sekademi.spotufi.di.SongPlayer.downloadProgress(song.url)
        ...
        Column(modifier = Modifier.navigationBarsPadding()) {
            ...
            SongMenuRow(
                ...
                if (downloaded) {
                    ...
                    downloadingNow = true
                    io.github.sekademi.spotufi.di.SongPlayer.downloadSong(song, context) { ok ->
