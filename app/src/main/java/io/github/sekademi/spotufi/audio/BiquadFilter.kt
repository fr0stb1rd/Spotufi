 ...
 * Based on Robert Bristow-Johnson's Audio EQ Cookbook. Ported from SimpMusic.
 *
 * Used for real-time DJ-style crossfade transitions:
 * - Outgoing track: LPF sweeps cutoff from 20 kHz -> ~200 Hz (treble fades, becomes muffled)
 * - Incoming track: HPF sweeps cutoff from ~2 kHz -> 20 Hz (bass fills in gradually)
 *
 * Two cascaded Butterworth stages → 4th order (24 dB/octave), like a pro DJ mixer.
 * Stereo with independent per-channel state. Thread-safe coefficient updates.
 */
class BiquadFilter {
