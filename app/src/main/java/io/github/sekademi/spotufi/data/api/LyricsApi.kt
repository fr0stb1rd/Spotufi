...
object LyricsApi {
    ...
    fun registerTracks(songs: List<io.github.sekademi.spotufi.data.entity.SongsModel>) {
        for (s in songs) {
    ...
    fun prefetch(title: String, artist: String, album: String, durationSec: Int = 0) {
        ...
        val ctx = io.github.sekademi.spotufi.MyApplication.instance
        io.github.sekademi.spotufi.data.preferences.getCachedLyrics(ctx, key)?.let {
            cache[key] = it
            return
        }
    ...
    suspend fun fetch(title: String, artist: String, album: String, durationSec: Int): Lyrics? {
        ...
        if (result != null) {
            io.github.sekademi.spotufi.data.preferences.cacheLyrics(ctx, key, result)
        }
        return result
    }
    ...
    private suspend fun fromSpotify(key: String): Lyrics? {
        ...
        runCatching {
            SpotifyTokenProvider.ensureToken(io.github.sekademi.spotufi.MyApplication.instance)
        }
