...
fun getDownloadFolderName(c: Context): String =
    prefs(c).getString(KEY_DOWNLOAD_FOLDER, DEFAULT_DOWNLOAD_FOLDER).orEmpty().ifBlank { DEFAULT_DOWNLOAD_FOLDER }

fun setDownloadFolderName(c: Context, name: String) =
    prefs(c).edit().putString(KEY_DOWNLOAD_FOLDER, sanitizeFolderName(name)).apply()

fun resetDownloadFolderName(c: Context) =
    prefs(c).edit().remove(KEY_DOWNLOAD_FOLDER).apply()

private fun sanitizeFolderName(name: String): String =
    name.replace(Regex("[/\\\\:*?\"<>|]"), "_").trim().take(64).ifBlank { DEFAULT_DOWNLOAD_FOLDER }
